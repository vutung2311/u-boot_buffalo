 ...
 *  - disabled interrupts.
 */
typedef void boot_os_Fcn(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[],
...
# error Unknown CPU type
	...
	case IH_COMP_GZIP:
		...
		if (gunzip((void *) ntohl(hdr->ih_load), unc_len, (uchar *) data, &len)
				!= 0) {
		...
		case IH_COMP_LZMA: {
			...
			i = lzmaBuffToBuffDecompress((void *)ntohl(hdr->ih_load), &lzma_len,
					(uchar *)data, len);
			len = lzma_len;
			if (i != SZ_OK) {
...
static void do_bootm_netbsd(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[],
	...
	loader = (void (*)(bd_t *, image_header_t *, char *, char *)) ntohl(
			hdr->ih_ep);
...
static void do_bootm_rtems(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[],
		ulong addr, ulong *len_ptr, int verify) {
	image_header_t *hdr = &header;
